...
def stitch2(left_img, right_img):
    # extract SIFT keypoints
    key_points1, descriptor1, key_points2, descriptor2 = get_keypoint(left_img, right_img)
...
def stitch_panorama(img1, img2):
    ...
    points2_transformed = cv2.perspectiveTransform(points2, homography_matrix)
    
    # Combine points to calculate bounds of the stitched image
    list_of_points = np.concatenate((points1, points2_transformed), axis=0)
    [x_min, y_min] = np.int32(list_of_points.min(axis=0).ravel() - 0.5)
    [x_max, y_max] = np.int32(list_of_points.max(axis=0).ravel() + 0.5)
    
    # Create the translation homography matrix to shift images into the final output canvas
    translation_dist = [-x_min, -y_min]
    H_translation = np.array([[1, 0, translation_dist[0]], [0, 1, translation_dist[1]], [0, 0, 1]], dtype=np.float32)
    
    # Warp the first image with the translation homography
    output_img = cv2.warpPerspective(img1, H_translation.dot(homography_matrix), (x_max - x_min, y_max - y_min))
    
    # Warp the second image into the output canvas
    img2_warped = cv2.warpPerspective(img2, H_translation, (x_max - x_min, y_max - y_min))
    
    # Combine the warped images by overlaying img2_warped onto output_img
    mask = (img2_warped > 0)  # Mask to prevent overwriting areas of img1
    output_img[mask] = img2_warped[mask]
...
if __name__ == "__main__":
    ...
    result_img2 = stitch(img_1, img_2)
    final_result = stitch_panorama(result_img2, result_img2)
